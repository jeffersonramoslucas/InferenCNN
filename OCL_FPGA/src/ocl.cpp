...
bool Ocl::fileExists(const char *file_name) {
#ifdef _WIN32 // Windows
...
std::string Ocl::getDeviceName(cl_device_id did) {
  cl_int status;
...
std::string Ocl::getBoardBinaryFile(const char *prefix, cl_device_id device) {
  ...
  if(end != std::string::npos) {
    ...
    file_name = std::string(prefix) + "_" + board_name + "_" + "141" + ".aocx";
    if(fileExists(file_name.c_str())) {
      return file_name;
    }
  }
...
unsigned char* Ocl::loadBinaryFile(const char *file_name, size_t *size) {
  // Open the File
  FILE* fp;
#ifdef _WIN32
...
cl_program Ocl::createProgramFromBinary(cl_context context, const char *binary_file_name, const cl_device_id *devices, unsigned num_devices) {
  ...
  //vector< unsigned char* > binary(binary_file);
  ...
  unsigned char** binaries = (unsigned char **) malloc(sizeof(unsigned char*)*num_devices);
  ...
  cl_int status;
  cl_int* binary_status = (cl_int*) malloc(sizeof(cl_int)*num_devices);
...
bool Ocl::init_opencl()
{
	...
	cl_device_type device_type = CL_DEVICE_TYPE_ALL;//(this->use_gpu == 1) ? CL_DEVICE_TYPE_GPU : CL_DEVICE_TYPE_CPU;
	get_devices(device_type);

	printf("Device number: %d\n", this->num_devices);

	// Create the context.
	context = clCreateContext(NULL, this->num_devices, this->devices, &oclContextCallback, NULL, &status);
	checkError(status, "Failed to create context");

	FILE* log;
	char buildLog[16 * PARAM_K_BYTE];

	if(this->pltaform_name == "altera")
	{
		std::string binary_file;

		// Create the program for all device. Use the first device as the
		// representative device (assuming all device are of the same type).
		binary_file = getBoardBinaryFile("cnn_cu2", devices[0]);
		printf("Using AOCX: %s\n", binary_file.c_str());
		this->prog_conv = createProgramFromBinary(context, binary_file.c_str(), devices, num_devices);
		// Build the program that was just created.
		status = clBuildProgram(this->prog_conv, 0, NULL, "", NULL, NULL);
		checkError(status, "Failed to build program");
		
		/*binary_file = getBoardBinaryFile("conv", devices[0]);
		printf("Using AOCX: %s\n", binary_file.c_str());
		this->prog_conv_depth = createProgramFromBinary(context, binary_file.c_str(), devices, num_devices);
		// Build the program that was just created.
		status = clBuildProgram(this->prog_conv_depth, 0, NULL, "", NULL, NULL);
		checkError(status, "Failed to build program");*/
	}
	else
	{
		// Create program with source
		this->prog_conv = creat_program();

		// Build program
		status = clBuildProgram(this->prog_conv, num_devices, devices, NULL, NULL, NULL);
	...
	clGetProgramBuildInfo(	this->prog_conv,
							devices[0],
