...
float* initialize_buffer(std::size_t num, std::size_t size ){
	//float* initialize = (float*) calloc(num,size);

	float* initialize = (float*)_aligned_malloc( sizeof(float) * num,  ALIGNMENT);
	memset (initialize, 0, sizeof(float) * num);
...
bool Net::init_problemInception()
{
	...
	*/
	...
	for (unsigned i = 0; i < ocl.num_devices; ++i) {
		...
		// Kernel conv.
		ocl.kernel_conv[i] = clCreateKernel(ocl.prog_conv, "cnn_conv_teste", &status);
		ocl.checkError(status, "Failed to create kernel cnn_conv_canonic_layer");

		// Kernel depth.
		ocl.kernel_depth[i] = clCreateKernel(ocl.prog_conv, "cnn_depth_layer", &status);
		ocl.checkError(status, "Failed to create kernel cnn_depth_layer");

		// Kernel batch_norm.
		ocl.kernel_batch_norm[i] = clCreateKernel(ocl.prog_conv, "cnn_batch_norm_layer", &status);
		ocl.checkError(status, "Failed to create kernel cnn_batch_norm_layer");

		// Kernel Pool.
		ocl.kernel_pool[i] = clCreateKernel(ocl.prog_conv, "cnn_pool_layer", &status);
		ocl.checkError(status, "Failed to create kernel cnn_pool_layer");

		// Kernel Padding.
		ocl.kernel_padding[i] = clCreateKernel(ocl.prog_conv, "cnn_padding_layer", &status);
		ocl.checkError(status, "Failed to create kernel cnn_padding_layer");

		// Kernel Relu.
		ocl.kernel_relu[i] = clCreateKernel(ocl.prog_conv, "cnn_relu_layer", &status);
		ocl.checkError(status, "Failed to create kernel cnn_relu_layer");

		// Kernel Elu.
		//ocl.kernel_elu[i] = clCreateKernel(ocl.prog_conv, "cnn_elu_layer", &status);
		//ocl.checkError(status, "Failed to create kernel cnn_elu_layer");

		// Kernel Sigmoide.
		//ocl.kernel_sigm[i] = clCreateKernel(ocl.prog_conv, "cnn_sigm_layer", &status);
		//ocl.checkError(status, "Failed to create kernel cnn_sigm_layer");

		// Kernel Tanh.
		//ocl.kernel_tanh[i] = clCreateKernel(ocl.prog_conv, "cnn_tanh_layer", &status);
		//ocl.checkError(status, "Failed to create kernel cnn_tanh_layer");

		// Kernel Matrix_Multi.
		ocl.kernel_matrix_mult[i] = clCreateKernel(ocl.prog_conv, "matrixMult", &status);
		ocl.checkError(status, "Failed to create kernel cnn_tanh_layer");

		// Kernel deep.
		//ocl.kernel_deep[i] = clCreateKernel(ocl.prog_conv, "cnn_conv_deep_layer", &status);
		//ocl.checkError(status, "Failed to create kernel cnn_deep_layer");

		// Kernel deep.
		//ocl.kernel_add[i] = clCreateKernel(ocl.prog_conv, "cnn_add_layer", &status);
...
bool Net::init_problem()
{
	...
	for (unsigned i = 0; i < ocl.num_devices; ++i) {
		...
		// Kernel conv.
		ocl.kernel_conv[i] = clCreateKernel(ocl.prog_conv, "cnn_conv_canonic_layer", &status);
		ocl.checkError(status, "Failed to create kernel cnn_conv_canonic_layer");
		...
		// Kernel Matrix_Multi.
		ocl.kernel_matrix_mult[i] = clCreateKernel(ocl.prog_conv, "cnn_matrixMul_layer", &status);
		ocl.checkError(status, "Failed to create kernel cnn_tanh_layer");
...
//		status = clSetKernelArg(ocl.kernel_conv[i], argi_conv++, 8*kernel_rows*kernel_cols*sizeof(float), NULL);
//		ocl.checkError(status, "Failed to set argument %d", argi_conv - 1);
		...
		//status = clSetKernelArg(ocl.kernel_conv[i], argi_conv++,sizeof(cl_short), &ondepth);
		//ocl.checkError(status, "Failed to set argument %d", argi_conv - 1);
		...
		/*if (ocl.n_per_device[i] > ocl.max_work_itens_size)
		{
			...
			vec_global[0] = (int)ceil((float)ocl.n_per_device[i]/ocl.max_work_itens_size)*ocl.n_per_device[i];
...
void Net::convolutionDepthOcl(cl_mem &input_conv, cl_mem &output_conv, cl_mem &kernel_vector, int rows, int cols, int kernel_rows, int kernel_cols, short kernel_amount, short kernel_depth, int featuresX, int featuresY, short strideX, short strideY)
{
	...
	//	ocl.checkError(status, "Failed to set argument %d", argi_conv - 1);
        ...
        ocl.checkError(status, "Failed to launch kernel");
        clFinish(ocl.queue[i]);
        //DepthEnd_NDRangeKernelTimer += (ocl.getCurrentTimestamp() - DepthStart_NDRangeKernelTimer);
...
void Net::batchNormOcl(cl_mem &input_padd, cl_mem &output_padd, cl_mem &kernel_vector, short kernel_depth, short rows, short cols)
{
	...
	for (unsigned i = 0; i < ocl.num_devices; ++i)
	{
		...
		//status = clSetKernelArg(ocl.kernel_batch_norm[i], argi_batch_norm++, sizeof(cl_short), &kernel_depth);
		//ocl.checkError(status, "Failed to set argument %d", argi_batch_norm - 1);
...
//            printf("chegou %d %d",id_conf,ocl.featuresMaps_buf.size() - 2 - params_.NumHiddenLayers - 1);
//            getchar();
			//####################### FULLY CONECTED #########################
			{
...
void Net::convAndPoolingInception(int rows, int cols, int* layer)
{
	...
	{
		status = clEnqueueWriteBuffer(	ocl.queue[0],
						ocl.featuresMaps_buf[id + 1],
						1, 0,
						sizeof(float) * featuresSizesX[id + 1] * featuresSizesY[id + 1] * featuresDeep[id + 1],
						initialize_buffer(featuresSizesX[id + 1] * featuresSizesY[id + 1] * featuresDeep[id + 1],sizeof(float)),
						0, 0, NULL);
		clFinish(ocl.queue[0]);
		ocl.checkError(status, "Failed to clEnqueueWriteBuffer samples_vector");
		id++;
...
//                                    0, NULL, NULL);
				...
				convolutionOcl2(	output_pad, ocl.featuresMaps_buf[id_conf + 1], this->ConvLayers[id_conf]->W, this->ConvLayers[id_conf]->b,
								featuresSizesX[id_conf] + pad_top + pad_bottom, featuresSizesY[id_conf] + pad_left + pad_rigth, kernel_rows, kernel_cols,
								ker_amount, featuresDeep[id_conf], featuresX, featuresY,
								conv_strideX, conv_strideY);
				...
				//getchar();
				//in = ocl.getCurrentTimestamp();
				convolutionOcl2(ocl.featuresMaps_buf[id_conf], ocl.featuresMaps_buf[id_conf + 1], this->ConvLayers[id_conf]->W, this->ConvLayers[id_conf]->b,
					rows, cols, kernel_rows, kernel_cols,
					ker_amount, ker_deep, featuresX, featuresY,
					conv_strideX, conv_strideY);
				//ou += (ocl.getCurrentTimestamp() - in);
...
bool Net::run_problem()
{
	...
	for (int index_samples = 0; index_samples < nsamples; index_samples += batchSize)
	{
		if (nsamples - index_samples < batchSize)
		{
			batchSize = nsamples - index_samples;
		...
		for (int index_batch = 0; index_batch < batchSize; index_batch++)
		{
			...
			float* init_sample = this->io.samples[index_batch].reshape(0, 1).ptr< float >(0);
			for (unsigned i = 0; i < ocl.num_devices; ++i)
			{
				// Input buffers.
				ocl.input_img_buf[i] = clCreateBuffer(ocl.context, CL_MEM_READ_WRITE | CL_MEM_ALLOC_HOST_PTR | CL_MEM_COPY_HOST_PTR,
			...
			while (strstr(io.conf_net[id_conf], "flatten") == NULL)
			{
				...
				if (strstr(io.conf_net[id_conf], "conv"))
				{
					...
					bool check_valid = strstr(this->ConvLayers[cl]->padding, "valid");

					short kernel_depth = this->KernelDepth[cl];
					short kernel_amount = this->KernelAmount[cl];

					float* bias_vector = (float*)_aligned_malloc(sizeof(float) * kernel_amount, ALIGNMENT);
					//for (int kerA = 0; kerA < kernel_amount; kerA++)
					//	bias_vector[kerA] = this->ConvLayers[cl]->layer[kerA]->b;

					float* kernel_vector = (float*)_aligned_malloc(sizeof(float) * kernel_amount * kernel_depth * kernel_cols * kernel_rows, ALIGNMENT);
