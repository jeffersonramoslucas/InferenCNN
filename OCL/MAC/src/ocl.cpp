...
cl_platform_id Ocl::findPlatform(std::string platform_name_search) {
    ...
    else if (platform_name_search.find("apple") != std::string::npos)
        platform_name_search = "apple";
    else
...
cl_program Ocl::creat_program()
{
	...
	fp = fopen("/Users/jefferson.r.anjos/workspace/teste/ker/cnn.cl", "rb");
...
bool Ocl::init_opencl()
{
	...
	// Create the context.
	context = clCreateContext(NULL, this->num_devices, &this->devices[0], NULL, NULL, &status);
	...
	// Build program
	status = clBuildProgram(this->prog, this->num_devices, &this->devices[0], NULL, NULL, NULL);
    checkError(status, "Failed to create clBuildProgram");
