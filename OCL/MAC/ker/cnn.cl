				...
				)
{
	...
	for (int kerD = 0; kerD < ker_deph; kerD++)
	{
		...
		for (int l = 0; l < ker_rows; l++)
		{
			...
			for (int k = 0; k < ker_cols; k++)
				sum +=  ker_local[ID_ker_local++] * img[id_img++];
		}
		barrier(CLK_LOCAL_MEM_FENCE);
		*/
				...
				)
{
    ...
    size_t idconv = conv_out_rows*conv_out_cols;
    size_t idker = ker_rows*ker_cols;
    size_t size_img = img_rows*img_cols;
    

    size_t id_amount = ker_rows*ker_cols*ker_deph*global_id;
        

    size_t idxOut = 0;
    size_t id_deep = idker*(global_id%ker_deph);
    size_t id_img_deep = size_img*(global_id%ker_deph);
    ...
    for (i = 0; i < img_rows - ker_rows + 1; i = i + strideX)
    {
        for (j = 0; j < img_cols - ker_cols + 1; j = j + strideY)
        {
            ...
            for (l = 0; l < ker_rows; l++)
            {
                size_t id_ker = id_amount + id_deep + l*ker_cols;
                size_t id_img = id_img_deep + (i + l)*img_cols + j;
                
                for (k = 0; k < ker_cols; k++)
                    sum += (float) ker[(id_ker + k)] * img[id_img + k];  //THEANO ( ker_rows*ker_cols - 1 - )
            }
            conv_out[idxOut++] +=  sum;//sum;//
        }
    }
					...
					)
{
	...
	/*
	if ((index) / (img_cols*img_cols) == 1)
	{
		id_out = id_out + id_out + index + (index / img_cols)*(pad_left + pad_rigth) + (img_cols + pad_left + pad_rigth) * (3) -1;
	}
	else if ((index) / (img_cols*img_cols) == 2)
	{
		id_out = id_out + id_out + id_out + index + (index / img_cols)*(pad_left + pad_rigth) + (img_cols + pad_left + pad_rigth) * (3*2) -2;
	}
	else
		id_out = id_out + index + (index / img_cols)*(pad_left + pad_rigth);
		*/
                     ...
                     )
{
    ...
    size_t id_irows_vs_icols = globalId*img_rows*img_cols;
    size_t twice_cols = img_cols+img_cols;
    
    for(int i = 0; i< img_rows; i++)
    {
        size_t id_cols = id_irows_vs_icols + i*img_cols;
        
        for(int j = 0; j < img_cols; j++)
        {
            out[id_cols + j] = ((img[id_cols + j]-ker[img_cols+j])*ker[j])/sqrt(ker[twice_cols+j]);
        }
    }
}
/*
					...
					)
{
	...
	for(int d = 0; d < img_depth; d++)
	{
		for(int i = 0; i< img_rows; i++)
		{
			for(int j = 0; j < img_cols; j++)
			{
				out[d*img_rows*img_cols + i*img_cols + j] = ((img[d*img_rows*img_cols + i*img_cols + j] - ker[j + img_cols]) * ker[j]) / sqrt(ker[j + img_cols + img_cols]);
				...
				)
{
 ...
 __kernel void ndrconv(// Params Ports
              unsigned in_h,
              unsigned in_w,
              unsigned out_c,
              unsigned in_c,
               ...
               )
{
    ...
    for (int kerD = 0; kerD < ker_deph; kerD++)
    {
        ...
        for (int i = 0; i < ker_rows_vs_cols; i++)
            sum += ker[id_kernel + i] * img[id_image + (i % ker_cols) + (i / ker_cols)*img_cols];
        barrier(CLK_LOCAL_MEM_FENCE);
