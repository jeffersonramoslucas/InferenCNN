...
void Io::read_Mnist(){
	...
	if (file->is_open()){
		...
		for (int i = 0; i < this->params_.batchSize; ++i){
			Mat tpmat = Mat::zeros(this->params_.samples_rows, this->params_.samples_cols, CV_8UC1);
			for (int r = 0; r < this->params_.samples_rows; ++r){
				for (int c = 0; c < this->params_.samples_cols; ++c){
					unsigned char temp = 0;
					file->read((char*)&temp, sizeof(temp));
...
void Io::readDataCIFAR(){
	...
	for (int i = 0; i < this->samples.size(); i++){
		this->samples[i].convertTo(this->samples[i], CV_32FC1, 1.0, 0);
	}
}
